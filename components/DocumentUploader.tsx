...
function stageLabel(s: PipelineStage): string {
  switch (s) {
    ...
    case "extracting":
      return "Running extraction pipeline…";
    case "done":
...
export function DocumentUploader({
  ...
  function addFiles(incoming: FileList | null) {
    ...
    setFiles((prev) => {
      ...
      return [
        ...prev,
        ...newItems.filter((f) => !existingNames.has(f.file.name)),
      ];
    });
  }
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      const classRes: ClassifyDocumentsResponse =
        await classifyDocuments(sessionId);
  ...
  const running =
    stage === "uploading" || stage === "classifying" || stage === "extracting";
    ...
    <Card>
      <CardHeader className="pb-3">
        ...
        <p className="text-sm text-gray-500">
          Upload one or more files. The system will auto-classify and analyse
          them.
        </p>
      </CardHeader>
      ...
      <CardContent className="space-y-5">
        ...
        {uploadedCount > 0 && (
          <p className="text-xs text-gray-500">
            {uploadedCount} document{uploadedCount !== 1 ? "s" : ""} uploaded to
            session.
          </p>
        )}
